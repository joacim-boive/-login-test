...
import {
  ...
  reducer,
} from 'src/features/account/redux/getCustomerAccountTransactions';
...
describe('account/redux/getCustomerAccountTransactions', () => {
  ...
  it('dispatches success action when getCustomerAccountTransactions succeeds', () => {
    ...
    return store.dispatch(getCustomerAccountTransactions())
      .then(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when getCustomerAccountTransactions fails', () => {
    ...
    return store.dispatch(getCustomerAccountTransactions({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_FAILURE);
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_BEGIN correctly', () => {
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_SUCCESS correctly', () => {
    const prevState = { getCustomerAccountTransactionsPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerAccountTransactionsPending).to.be.false;
  });

  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_FAILURE correctly', () => {
    const prevState = { getCustomerAccountTransactionsPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerAccountTransactionsPending).to.be.false;
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_TRANSACTIONS_DISMISS_ERROR correctly', () => {
