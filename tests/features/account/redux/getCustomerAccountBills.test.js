...
import {
  getCustomerAccountBills,
  dismissGetCustomerAccountBillsError,
  reducer,
} from 'src/features/account/redux/getCustomerAccountBills';
...
describe('account/redux/getCustomerAccountBills', () => {
  ...
  it('dispatches success action when getCustomerAccountBills succeeds', () => {
    ...
    return store.dispatch(getCustomerAccountBills())
      .then(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when getCustomerAccountBills fails', () => {
    ...
    return store.dispatch(getCustomerAccountBills({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_FAILURE);
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_BEGIN correctly', () => {
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_SUCCESS correctly', () => {
    const prevState = { getCustomerAccountBillsPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerAccountBillsPending).to.be.false;
  });

  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_FAILURE correctly', () => {
    const prevState = { getCustomerAccountBillsPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerAccountBillsPending).to.be.false;
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_BILLS_DISMISS_ERROR correctly', () => {
