...
import {
  ...
  reducer,
} from 'src/features/account/redux/getCustomerAccountCards';
...
describe('account/redux/getCustomerAccountCards', () => {
  ...
  it('dispatches success action when getCustomerAccountCards succeeds', () => {
    ...
    return store.dispatch(getCustomerAccountCards())
      .then(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when getCustomerAccountCards fails', () => {
    ...
    return store.dispatch(getCustomerAccountCards({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_FAILURE);
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_BEGIN correctly', () => {
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_SUCCESS correctly', () => {
    const prevState = { getCustomerAccountCardsPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerAccountCardsPending).to.be.false;
  });

  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_FAILURE correctly', () => {
    const prevState = { getCustomerAccountCardsPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerAccountCardsPending).to.be.false;
  ...
  it('handles action type ACCOUNT_GET_CUSTOMER_ACCOUNT_CARDS_DISMISS_ERROR correctly', () => {
