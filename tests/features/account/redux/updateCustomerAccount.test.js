import configureMockStore from 'redux-mock-store';
import thunk from 'redux-thunk';
import nock from 'nock';
import { expect } from 'chai';

import {
  ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_BEGIN,
  ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_SUCCESS,
  ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_FAILURE,
  ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_DISMISS_ERROR,
} from 'src/features/account/redux/constants';

import {
  updateCustomerAccount,
  dismissUpdateCustomerAccountError,
  reducer,
} from 'src/features/account/redux/updateCustomerAccount';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('account/redux/updateCustomerAccount', () => {
  afterEach(() => {
    nock.cleanAll();
  });

  it('dispatches success action when updateCustomerAccount succeeds', () => {
    const store = mockStore({});

    return store.dispatch(updateCustomerAccount())
      .then(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_SUCCESS);
      });
  });

  it('dispatches failure action when updateCustomerAccount fails', () => {
    const store = mockStore({});

    return store.dispatch(updateCustomerAccount({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_BEGIN);
        expect(actions[1]).to.have.property('type', ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_FAILURE);
        expect(actions[1]).to.have.nested.property('data.error').that.exist;
      });
  });

  it('returns correct action by dismissUpdateCustomerAccountError', () => {
    const expectedAction = {
      type: ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_DISMISS_ERROR,
    };
    expect(dismissUpdateCustomerAccountError()).to.deep.equal(expectedAction);
  });

  it('handles action type ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_BEGIN correctly', () => {
    const prevState = { updateCustomerAccountPending: false };
    const state = reducer(
      prevState,
      { type: ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_BEGIN }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.updateCustomerAccountPending).to.be.true;
  });

  it('handles action type ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_SUCCESS correctly', () => {
    const prevState = { updateCustomerAccountPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.updateCustomerAccountPending).to.be.false;
  });

  it('handles action type ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_FAILURE correctly', () => {
    const prevState = { updateCustomerAccountPending: true };
    const state = reducer(
      prevState,
      { type: ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.updateCustomerAccountPending).to.be.false;
    expect(state.updateCustomerAccountError).to.exist;
  });

  it('handles action type ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_DISMISS_ERROR correctly', () => {
    const prevState = { updateCustomerAccountError: new Error('some error') };
    const state = reducer(
      prevState,
      { type: ACCOUNT_UPDATE_CUSTOMER_ACCOUNT_DISMISS_ERROR }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.updateCustomerAccountError).to.be.null;
  });
});
