...
import {
  ...
  reducer,
} from 'src/features/customer/redux/getCustomerExtraCardHolders';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('customer/redux/getCustomerExtraCardHolders', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when getCustomerExtraCardHolders succeeds', () => {
    ...
    return store.dispatch(getCustomerExtraCardHolders())
      .then(() => {
        ...
        expect(actions[0]).to.have.property('type', CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_BEGIN);
        expect(actions[1]).to.have.property('type', CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when getCustomerExtraCardHolders fails', () => {
    ...
    return store.dispatch(getCustomerExtraCardHolders({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_BEGIN);
        expect(actions[1]).to.have.property('type', CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_FAILURE);
  ...
  it('handles action type CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_BEGIN correctly', () => {
  ...
  it('handles action type CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_SUCCESS correctly', () => {
    const prevState = { getCustomerExtraCardHoldersPending: true };
    const state = reducer(
      prevState,
      { type: CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerExtraCardHoldersPending).to.be.false;
  });

  it('handles action type CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_FAILURE correctly', () => {
    const prevState = { getCustomerExtraCardHoldersPending: true };
    const state = reducer(
      prevState,
      { type: CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.getCustomerExtraCardHoldersPending).to.be.false;
  ...
  it('handles action type CUSTOMER_GET_CUSTOMER_EXTRA_CARD_HOLDERS_DISMISS_ERROR correctly', () => {
